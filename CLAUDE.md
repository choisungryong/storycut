...
- **버그 수정 시 영향도 분석 필수**: 데이터 흐름을 입구(API 요청)부터 출구(최종 저장/응답)까지 전체 write 경로를 추적한다. 읽기 쪽만 고치고 쓰기 쪽을 검증하지 않는 것은 금지. 특히 manifest, user_id 등 여러 곳에서 쓰이는 데이터는 모든 write 지점을 빠짐없이 확인한다.
- **PRD 환경**: 코드 수정 전 반드시 수정 계획을 공유하고 승인받은 후 진행한다.
