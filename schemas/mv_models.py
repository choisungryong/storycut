...
class MVCharacter(BaseModel):
    ...
    unique_features: str = Field(default="", description="고유 식별 특징 -- 점, 흉터, 특이한 눈 색깔, 헤어 악세서리, 문신 등 3가지 이상")
...
class MusicAnalysis(BaseModel):
    ...
    stt_sentences: Optional[List[Dict[str, Any]]] = Field(None, description="Raw STT 문장 [{t: 초, text: 인식텍스트}] (타이밍 에디터용)")
...
class MVScene(BaseModel):
    ...
    pexels_video_id: Optional[int] = Field(None, description="Pexels 영상 ID (재생성 시 중복 방지용)")
